const content = {
  ...
  calendar: {
    ...
    title: "Сегодня открыт третий набор",
    copy:
      "Нажмите на доступную капсулу, чтобы раскрыть содержимое третьего дня. Сегодня внутри лежат новые эффекты, напрямую связанные с происходящими событиями.",
    dropdownLabel: "Содержимое набора 03",
    slots: [
      {
        id: "set-01",
        label: "Набор 01",
  ...
  news: [
    {
      icon: "🐇",
      title: "Повышение «случайной отзывчивости» в общественных местах",
      text: [
        "Жители Пентаграм-сити отмечают, что всё чаще становятся объектами ненавязчивой, но настойчивой заботы: им придерживают двери, уступают очереди и даже предлагают помощь с мелкими поручениями без явной причины.",
        "Сами «заботящиеся» утверждают, что «просто проходили мимо и почувствовали импульс».",
        "Отказаться сложно — люди выглядят очень убедительно.",
      ],
    },
    {
      icon: "🥚",
      title: "Праздничные объекты начали имитировать социальное поведение",
      text: [
        "Яйца, обнаруженные ранее, теперь не просто перемещаются — они образуют небольшие группы. Зафиксированы случаи, когда два или три объекта «собираются» вместе, словно обсуждая что-то.",
        "Попытки разъединить их не приводят к успеху — они возвращаются друг к другу в течение часа.",
        "Природа явления остаётся неясной.",
      ],
    },
    {
      icon: "🌸",
      title: "В отеле Hazbin открылась «Комната тихого единения»",
      text: [
        "В рамках продолжающихся мероприятий по сплочению сегодня заработало новое пространство — небольшая комната с приглушённым светом, мягкими креслами и подозрительно умиротворяющей музыкой.",
        "Посетители сообщают, что после пяти минут пребывания испытывают трудности с агрессией и желание сказать кому-то что-то приятное.",
        "Персонал отеля рекомендует не задерживаться дольше пятнадцати минут «во избежание необратимых последствий».",
      ],
    },
  ],
  dice: {
    ...
    ranges: [
      {
        ...
        text:
          "Вам предлагают зайти в «Комнату тихого единения». Вы можете отказаться, но мысль о том, что там происходит, не даёт покоя. Если решитесь — постарайтесь не сидеть там дольше пяти минут. Или сидите. Кто знает, что будет.",
      },
      {
        min: 6,
        max: 10,
        ...
        text:
          "К вам присоединяется одно из «социальных» яиц. Оно просто находится рядом. Неотступно. Иногда кажется, что оно оценивает ваши действия. Вам придётся принимать решения с оглядкой на этот безмолвный объект до конца дня.",
      },
      {
        min: 11,
        max: 15,
        ...
        icon: "🧸",
        title: "Принудительный комплимент",
        text:
          "Вы внезапно чувствуете непреодолимое желание сказать кому-то рядом искренний комплимент. Не просто вежливость, а что-то настоящее. Сопротивление вызывает лёгкое головокружение. Лучше просто сделайте это.",
      },
      {
        min: 16,
        max: 20,
        ...
        icon: "🍞",
        title: "Эмпатическая волна",
        text:
          "Вас накрывает странное состояние: вы начинаете понимать эмоции окружающих немного лучше, чем хотели бы. Кто-то грустит — вам тоже не по себе. Кто-то радуется — вы почти счастливы. Это продлится до конца дня. Держитесь.",
      },
    ],
  },
  footer: {
